...
func Test() {
	...
	queryDB(context.Background(), Notion_DB, notion.DatabaseQuery{
		Filter: notion.DatabaseQueryFilter{
			...
			Number: notion.NumberDatabaseQueryFilter{
				Equals:               new(int),
				DoesNotEqual:         new(int),
				GreaterThan:          new(int),
				LessThan:             new(int),
				GreaterThanOrEqualTo: new(int),
				LessThanOrEqualTo:    new(int),
				IsEmpty:              false,
				IsNotEmpty:           false,
			},
			Checkbox: notion.CheckboxDatabaseQueryFilter{
				Equals:       new(bool),
				DoesNotEqual: new(bool),
			},
			...
			Files: notion.FilesDatabaseQueryFilter{
				IsEmpty:    false,
				IsNotEmpty: false,
			},
...
type DatabaseQuery struct {
	Filter      notion.DatabaseQueryFilter `json:"filter,omitempty"`
	Sorts       []notion.DatabaseQuerySort `json:"sorts,omitempty"`
	StartCursor string                     `json:"start_cursor,omitempty"`
	PageSize    int                        `json:"page_size,omitempty"`
}

func queryDB(ctx context.Context, id string, query notion.DatabaseQuery) (result notion.DatabaseQueryResponse, err error) {
	client := notion.NewClient(Notion_Key)
	// var none nil;
	// none = nil
	res, err := client.QueryDatabase(context.Background(), Notion_DB, notion.DatabaseQuery{
		Filter: notion.DatabaseQueryFilter{
			Property:    "",
			Text:        notion.TextDatabaseQueryFilter{},
			Number:      notion.NumberDatabaseQueryFilter{},
